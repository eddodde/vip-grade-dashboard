...
section("6 · 심화: 어디서 어디로 (From → To)",
        ...
        "등급으로 어떻게 옮겨갔는지(유지·승급·하락). 기본은 VIP core, 위 라디오로 범위 변경.",
        anchor="sec-matrix")
mm = st.select_slider("기준월", options=YMS, value=ym1, format_func=ymlab,
                      key="mxm")
mx = mat[mat.YM == mm]
SCOPES = {"VIP core (BK·SV·GD)": {"BK", "SV", "GD"},
          "VIP 전체 (+PT·SP)": set(VIP_ALL),
          "전체 (일반 포함)": set(ORDER)}
scope = st.radio("등급 범위", list(SCOPES), index=0, horizontal=True, key="mxscope")
SCG = [g for g in ORDER if g in SCOPES[scope]]      # 상→하 순
mxs = mx[mx.FROM.isin(SCG) & mx.TO.isin(SCG)]
how_to("두 차트 모두 같은 데이터(전월→당월 이동)를 보여줍니다. "
       "RD·PP(일반)는 규모가 압도적이라 기본은 <b>VIP core(BK·SV·GD)</b>만 — 위 라디오로 범위 변경.<br>"
       ...
       "<span style='color:#95A5A6'>회색=유지(같은 등급)</span>. "
       "가로로 곧게 가는 회색 띠가 그 등급에 머문 사람.<br>"
       "• 등급은 <b>위에서 아래로 상위→하위</b> 순.")
# 자동 인사이트: 이번 달 가장 큰 '이동'(유지 제외) 3건
_mv = mxs[mxs.FROM != mxs.TO].nlargest(3, "CNT")
if len(_mv):
    _txt = ", ".join(
        f"<b>{r.FROM}→{r.TO}</b> {r.CNT:,.0f}명"
        f"(<span style='color:{DIRCOLOR[r.dir]}'>{r.dir}</span>)"
        for r in _mv.itertuples())
    insight(f"{ymlab(mm)} {scope} 내 가장 큰 이동 — {_txt}.")
c1, c2 = st.columns(2)
with c1:
    excl = st.checkbox("유지(대각) 제외", value=False, key="hk")
    h = mxs[mxs.FROM != mxs.TO] if excl else mxs
    piv = (h.pivot_table(index="FROM", columns="TO", values="CNT", aggfunc="sum")
             .reindex(index=SCG, columns=SCG))
...
with c2:
    s = mxs                                        # 유지(대각) 포함
    L = {g: i for i, g in enumerate(SCG)}
    R = {g: i + len(SCG) for i, g in enumerate(SCG)}
    lc = s["dir"].map({"승급": "rgba(39,174,96,.55)", "하락": "rgba(231,76,60,.55)",
                       "유지": "rgba(149,165,166,.35)"})
    fig = go.Figure(go.Sankey(
        node=dict(label=[f"{g}·전월" for g in SCG] + [f"{g}·당월" for g in SCG],
                  color=[GCOLOR[g] for g in SCG] * 2, pad=14, thickness=16),
    ...
    st.plotly_chart(fig, use_container_width=True,
                    config={"displayModeBar": False})
    st.caption("좌=전월, 우=당월. 초록=승급·빨강=하락·회색=유지.")
