...
def render_aging():
    ...
    insight(
        "🎯 <b>시사점 · 해야 할 일</b><br>"
        f"① <b>하락가망(12M) {risk_cnt:,.0f}명 — 추가구매 1건 유도로 등급 방어(ROI 핀포인트).</b> "
        "등급은 직전 12개월 구매로 산정되는데, 이 구간은 VIP 12개월차라 "
        "<b>가입 후 추가 구매가 없으면 최초 자격 구매가 산정창에서 빠져 다음 달 하락</b>합니다. "
        f"그런데 월방문율 {risk_act*100:.0f}%로 온보딩·안정화 평균({mid_act*100:.0f}%)보다 {_save} — "
        "<b>앱은 잘 쓰는데 구매만 안 한</b> 상태라, 쿠폰·리마인드로 구매 1건만 유도하면 "
        "산정창에 다시 들어와 살릴 여지가 큽니다.<br>"
...
mm = st.select_slider("기준월", options=YMS, value=ym1, format_func=ymlab,
                      key="mxm")
mx = mat[mat.YM == mm]
how_to("두 차트 모두 같은 데이터(전월→당월 이동)를 다르게 보여줍니다.<br>"
       "• <b>왼쪽 히트맵</b>: <b>행=전월 등급, 열=당월 등급</b>. 칸의 숫자=이동 인원, "
       "색이 진할수록 많음. 예) 'SV행 × BK열' 칸 = 지난달 SV였다가 이번달 BK로 간 사람.<br>"
       "• <b>오른쪽 Sankey</b>: 왼쪽=전월, 오른쪽=당월. 띠 두께=인원, "
       "<span style='color:#27AE60'>초록=승급</span>, "
       "<span style='color:#E74C3C'>빨강=하락</span>.<br>"
       "• 등급은 <b>위에서 아래로 SP→RD(상위→하위)</b> 순으로 배치됩니다.")
# 자동 인사이트: 이번 달 가장 큰 이동 3건
...
if len(_mv):
    ...
    insight(f"{ymlab(mm)} 가장 큰 이동 — {_txt}. "
            "대각선(유지)을 빼면 어디서 빠지고 어디로 가는지가 드러납니다.")
c1, c2 = st.columns(2)
with c1:
