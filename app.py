...
def render_integrated():
    ...
    _op = [
        ...
        ("핵심 액션", "방문 트리거: 핫딜·콘텐츠·출석/열람 리워드",
         "PP 상위 승급 넛지 + 신규 웰컴·온보딩 저니"),
        ("타겟", "방문빈도 꺾인 장기유지(휴면 전조)", "일반 PP 상위 + 신규 VIP 0~5M"),
        ("KPI", "주간 DAU · Stickiness(DAU/MAU)", "일반→VIP 전환율 · 신규 3M 잔존"),
    ]
    _oprows = "".join(
        f"<tr><td class='seg'>{k}</td><td>{a}</td><td>{b}</td></tr>"
        for k, a, b in _op)
    st.markdown(
    ...
    ph[0].markdown(
        ...
        unsafe_allow_html=True)
    ph[1].markdown(
        '<div class="ccard why"><span class="t">4~8주 · 확산</span><ul>'
        '<li>레버1: <b>반응 세그먼트 스케일</b>·채널 최적화</li>'
        '<li>레버2: 온보딩 저니 <b>런칭</b>, PP 프로모션 가동</li></ul></div>',
        unsafe_allow_html=True)
    ph[2].markdown(
        '<div class="ccard risk"><span class="t">8~12주 · 검증</span><ul>'
    ...
    # ── VIP 고객 저니 × CRM 캠페인 맵 ──
    st.markdown("##### 🗺 종합: 고객 여정 위에서 본 CRM 액션")
    insight(
        "위 <b>액션 플랜(①~④)</b>과 <b>지금 운영 중인 캠페인</b>을 <b>고객 여정 한 줄에 얹으면</b>, "
        "우리 프로그램이 <b>어느 단계를 커버하고 어디가 비었는지</b>가 한눈에 드러납니다. "
        "결론은 앞과 같아요 — <b>양 끝(유치·재활성화)은 촘촘한데 가운데(정착·갱신)가 공백</b>. ↓")
    st.markdown(JOURNEY_MAP_HTML, unsafe_allow_html=True)
    st.caption("녹아든 액션: 유치=①승급유도(운영중) · 신규진입/정착/갱신=②온보딩·③방문습관·"
               "하락가망방어(★공백) · 유지/재활성화=재참여·미방문(운영중+◇고도화). "
